...
public class ClientSyncTool {
	...
	public int startToSynch(UserAccount tCurUser, String pCommand){
...
//		WebResource webResource = client.resource("http://localhost/bigbang/useraccounts/" + pCommand);		//have to add an value: /jsonArray, otherwise, the
		WebResource webResource = client.resource("http://cn.sharethegoodones.com/useraccounts/" + pCommand);		//have to add an value: /{pCommand}, to distinguish
...
//		webResource.method("POST");	//this method will call onto the remote server, duplicated with webResource.post();
		...
		ClientResponse response = webResource.type("application/json").post(ClientResponse.class, synchnizationManager.getRecentlyAddedContent(tCurUser.getName(), pCommand));
