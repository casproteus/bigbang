...
public class ClientSyncTool {
	...
	public int startToSynch(UserAccount tCurUser){
...
//		WebResource webResource = client.resource("http://localhost/bigbang/useraccounts/jsonArray");		//have to add an value: /jsonArray, otherwise, the
		WebResource webResource = client.resource("http://cn.sharethegoodones.com/useraccounts/jsonArray");		//have to add an value: /jsonArray, otherwise, the
...
//		webResource.method("POST");	//this method will call onto the remote server, duplicated with webResource.post();
		...
		ClientResponse response = webResource.type("application/json").post(ClientResponse.class, synchnizationManager.getRecentlyAddedContent(tCurUser.getName()));
//		ClientResponse response = webResource.post(ClientResponse.class);
		if(200 == response.getStatus()){
			...
			if(tList.size() != 6)
				return -1;
			return synchnizationManager.saveContentIntoLocalDB(tList);
		}
		return -1;
	}
