...
public class SynchnizationManager {
	...
	// TODO: Bug1: the local useraccount should not update remote useraccount,
	// because the romote one might
	// be modified already and is newer than local ones. so should check the last
	// update time to know which should be
	// modified.
	...
	public boolean saveUserAccountToLocalDB(String pJsonStr) {
		// @NOTE, have to get the uaList out side of the method, because the method will
		// be called recurrencely
		userAccountList = new JSONDeserializer<List<UserAccount>>().use(null, ArrayList.class)
				.use("values", UserAccount.class).deserialize(pJsonStr);
	...
	private boolean startToSaveUAIntoDB(List<UserAccount> pList) {
		...
		for (int i = 0; i < pList.size(); i++) {
			...
			// Replace the element with the ones in tUserAccountList, because the infomation
			// can be not complete as
	...
	public boolean saveTagsToLocalDB(String pJsonStr) {
		List<BigTag> tBigTagList = new JSONDeserializer<List<BigTag>>().use(null, ArrayList.class)
				.use("values", BigTag.class).deserialize(pJsonStr);
		stackUAprocessing.clear();
		stackUAprocessed.clear(); // can not be cleared in it's own method, cause the method can be iterated
									// several
									// times.

		tagMap.clear();
		System.out.println(
				"start to save BigTags! there are [" + tBigTagList.size() + "] items to save---------------------");
		...
		for (int i = 0; i < tBigTagList.size(); i++) {
			...
			if (tList != null && tList.size() > 0) {
				...
				if (noMatch) { // not exist, add new.
					pTag.setId(null);
					pTag.persist(); // don't need to set publisher, because it was saved as string instead of an
									// object.
				}
	...
	public boolean saveMessagesToLocalDB(String pJsonStr) {
		List<Message> tMessageList = new JSONDeserializer<List<Message>>().use(null, ArrayList.class)
				.use("values", Message.class).deserialize(pJsonStr);
		messageMap.clear();
		System.out.println(
				"start to save Messages! there are [" + tMessageList.size() + "] items to save---------------------");
	...
	public boolean saveBlogsToLocalDB(String pJsonStr) {
		List<Twitter> tBlogList = new JSONDeserializer<List<Twitter>>().use(null, ArrayList.class)
				.use("values", Twitter.class).deserialize(pJsonStr);
		twitterMap.clear();
		System.out.println(
				"start to save Blogs! there are [" + tBlogList.size() + "] items to save---------------------");
		...
		// twitterMap.clear(); we don't clear it here, because the coming
		// startToBackUpRemarksToLocal will need this
		// cache.
		return true;
	}
	...
	public boolean saveRemarksToLocalDB(String pJsonStr) {
		List<Remark> tRemarkList = new JSONDeserializer<List<Remark>>().use(null, ArrayList.class)
				.use("values", Remark.class).deserialize(pJsonStr);
		remarkMap.clear();
		System.out.println(
				"start to save Remarks! there are [" + tRemarkList.size() + "] items to save---------------------");
		...
		for (int i = 0; i < tRemarkList.size(); i++) {
			...
			if (tList == null) {
				...
				tList = Remark.findRemarkByTwitter(tBlog,
	...
	public boolean saveBookmarksToLocalDB(String pJsonStr) {
		List<Content> tBookMarkList = new JSONDeserializer<List<Content>>().use(null, ArrayList.class)
				.use("values", Content.class).deserialize(pJsonStr);
		bookmarkMap.clear();
		System.out.println(
				"start to save contents! there are [" + tBookMarkList.size() + "] items to save---------------------");
	...
	private UserAccount findUserInLocalDB(UserAccount pUA) {
		if (pUA == null) // some times, when string is big, some user (like Jane) might be parsered to
							// null.
			return null;
		UserAccount tUA = UserAccount.findUserAccountByName(pUA.getName());
		// Because already synced useraccount with server, so, if not in DB yet, then
		// must be unknow remarkers or
		// message senders,
