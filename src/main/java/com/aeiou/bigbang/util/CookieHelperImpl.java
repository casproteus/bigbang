...
public class CookieHelperImpl implements CookieHelper {
			...
			for (Cookie cookie : cookies) {
				if (cookie.getMaxAge() > 0 && key.equals(cookie.getName())) {
					if (log.isDebugEnabled()) {
						log.debug("Found cookie {}={} having maxAge={}", new Object[] { cookie.getName(), cookie.getValue(), cookie.getMaxAge()});
					}
	...
	@Override
	public boolean isOlder(String cookie1Key, String cookie2Key, boolean strict, HttpServletRequest request) {
		final Cookie[] cookies = request.getCookies();
		if (cookies != null) {
			int age1 = 0;
			int age2 = 0;
	...
	@Override
	public void deleteCookie(String key, HttpServletRequest request, HttpServletResponse response) {
		Cookie cookie = null;
