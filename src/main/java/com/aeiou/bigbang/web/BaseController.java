...
public class BaseController {
    ...
    /**
     * if the language is switched, or visiting a different owner's space, then re-init the texts on customisable areas
     * (header and footer)
     * 
     * @param pOwner
     * @param model
     * @param request
     */
    private void checkIfCusTextNeedTobeReInit(
            UserAccount pOwner,
            Model model,
            HttpServletRequest request) {
     ...
     * only when the local is in five supported lang, and the flag in customize table is set as "true", request.locale
     * will be returned.
     * 
     * @param request
     * @return
     */
    private String decideLangWithDefaultValue(
            HttpServletRequest request,
            String defaultLang) {
    ...
    /*
     * if it's normal user's main page, then need to reload his/her customise. in case he has his own customize. please
     * be noticced order, make sure admin's first, then user's customise, so user's can replace admins.
     */
    private List<Customize> reloadCustomizesToSession(
            UserAccount pOwner,
            HttpSession session) {
    ...
    /*
     * apply the language property, replace the value with the one under right key(with language mark). the logic, if
     * found key end with current lang, then use it to replace the one which is used in jspx page.
     */
    private void replaceValuesWithLang(
            List<Customize> customizes,
            String suffix,
            HttpSession session) {
        if (customizes == null) {
            return;
